...
public class goalkeeper2 : MonoBehaviour {
	...
	private float savetimeout=0.0f;  private float angle=0.0f;private float t=0.0f;
	...
	[HideInInspector]
	public bool justkicked=false;
	[HideInInspector]
	public float timer=0.0f; private float divespeed=0.0f;
	...
	void  Update (){
		...
		if(AI.shot>0 && acting==false && state!="saving" && state!="longsave" && Vector3.Distance(ball.transform.position,transform.position)<30)//short range save
		...
		{var vector=Vector3.Normalize(new Vector3(ball.transform.position.x,0.0f,0));if(state!="longsave")lstime=60;
			GetComponent<Rigidbody>().MovePosition(transform.position-vector*0.1f);state="longsave";LongSave();acting=true;}

		if(state=="longsave")
			LongSave();

		//if(state!="saving"){
		if( (Vector3.Distance(spot.position,ball.transform.position)>20 && acting==false && sos==false)
			||( Vector3.Distance(spot.position,ball.transform.position)<20 && AI.posession==2 && acting==false ) )  //p2 fix
    ...
    state="saving";*/
		...
		if(Ball.owner!=null){ //attacker passed all defence xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
			if(state=="idle"  && AI.posession==1 && Ball.owner.transform.position.z>player10.transform.position.z && 
		...
		{sos=false;}
		if(AI.posession!=1 || transform.position.z<23)
			sos=false;
		...
		if(state=="idle" && acting==false && 
			ball.transform.position.y>transform.position.y+2.2f && Vector3.Distance(transform.position,ball.transform.position)<2
		...
		if(Mathf.Abs(BallPoint.saveangle2)<45.0f && ball.transform.position.y>transform.position.y+2.4f && Vector3.Distance(ball.transform.position,transform.position)<2
			&& state=="saving")
			hands.enabled=true;
		else
		...
		if(state=="attack" && ball.transform.position.y>transform.position.y+2.0f && Vector3.Distance(ball.transform.position,transform.position)<5)
			hands.enabled=true;
		StayUpright();
	}// update xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

	void  StayUpright (){
		if(state!="saving" && (AI.posession==5 || AI.penalties==false) && state!="longsave"){
			if(transform.eulerAngles.z!=0.0f)
				transform.eulerAngles= new Vector3(transform.eulerAngles.x,transform.eulerAngles.y,0);
			if(transform.eulerAngles.x!=0.0f)
				transform.eulerAngles=new Vector3(0,transform.eulerAngles.y,transform.eulerAngles.z);
	...
	void  ReturnToPosition (){ if(AI.penalties)return;
		...
		else{if(GetComponent<Rigidbody>().velocity.magnitude<0.01f)
			GetComponent<Animation>().Play("idle");}
	}
	...
	void  GoForBall (){ if(AI.penalties)return; //rigidbody.isKinematic=true;
		...
		if(Vector3.Distance(ball.transform.position,spot.transform.position)<5)
	...
	void  OnCollisionEnter ( Collision other  ){ 
		...
		if( AI.posession==5 ||  AI.posession==6 || AI.posession==7 || state=="withball")return;
		if(other.gameObject.name=="ball")AI.cameoff="p2";  
	...
	void  OnCollisionStay ( Collision other  ){ if(other.gameObject.name=="ball")AI.SHOOT=false;
		if(other.gameObject.name=="ball" && (AI.posession==5 ||  AI.posession==6 || global.penalties==true) )return;

		if(other.gameObject.name=="ball" && kicked==true)
			return;

		if(other.gameObject.name=="ball" && state=="attack" && AI.shot<=0){ //p2 make sure grab from opponent
			if(inbox==true && ball.GetComponent<Rigidbody>().velocity.magnitude<300/control)
				Grab();
	...
	void  Grab (){  if(global.penalties)return;
		if( AI.posessor.tag!="Player2" || Ball.state=="free"){  //p2  don't grab from fellow teammate
			if(AI.posessor!=null && Ball.state!="free"){
				if(AI.posessor.GetComponent<player>().dribbling){
					if(Random.Range(0.0f,AI.posessor.GetComponent<player>().dribble/10.0f)<=1){
  ...
  animation.Play("save");GetComponent.<Animation>()["save"].speed=1.0f;*/
		...
		{   //if(GetComponent.<Animation>()["save"].normalizedTime<1.0f && !animation.IsPlaying("save"))animation.Play("save");
			transform.eulerAngles=new Vector3(transform.eulerAngles.x,transform.eulerAngles.y, Mathf.Lerp(angle,BallPoint.saveangle2,t) );
			//transform.eulerAngles.z=BallPoint.saveangle2;                                  
			t+=reflexes/30*Time.deltaTime;  //0 to 1
			Mathf.Clamp(divespeed,0.0f,1.0f);
			 divespeed=BallPoint.D2;
			Mathf.Clamp(divespeed,0.0f,1.0f);
			//transform.eulerAngles.z=BallPoint.saveangle;
			hands.enabled=true;  savedown+=1;
			if(Ball.yvel>0.2f && BallPoint.saveangle<30.0f)
		...
		if(savetimeout>=timeline && savetimeout<=10.0f/reflexes+0.1f && jumped==false)//jump
		...
		if(savetimeout>timeline && savetimeout<10/reflexes+0.5f && Vector3.Distance(ball.transform.position,spot.transform.position)<5){
			...
			// rigidbody.MovePosition(transform.position+Vector3.Normalize(Vector3(Ball.x2trajectory-transform.position.x,0,0))*diving/1000);
			// else
		...
		{state="idle";acting=false;hands.enabled=false;saved=false;savetimeout=0.0f;jumped=false;savedown=1000;}
	}
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void  AllBack (){
	...
	void  LongSave (){
		...
		if(Ball.xtrajectory!=null)
			GetComponent<Rigidbody>().MovePosition(transform.position+Vector3.Normalize(new Vector3(Ball.x2trajectory-transform.position.x,0,0))*speed/2000);
		//rigidbody.MovePosition(transform.position+Vector3.Normalize(it-transform.position)*speed*0.25f*Time.timeScale);
		if(lstime==1)
		...
		{
			transform.eulerAngles=new Vector3(transform.eulerAngles.x,transform.eulerAngles.y, BallPoint.saveangle2);                                  

			if(!GetComponent<Animation>().IsPlaying("save"))
				GetComponent<Animation>().Play("save");}
		var ballpos= new Vector3(ball.transform.position.x,0.83f,ball.transform.position.z);
		if( Vector3.Distance(ball.transform.position,transform.position)<5){
			GetComponent<Rigidbody>().MovePosition(transform.position+Vector3.Normalize(ballpos-transform.position)*speed*0.5f*Time.timeScale);}
	}
}
